    ...
    if (page == 'add' || page == 'edit' || page == 'view') {
        ...
        if (panelbody && panelbody.length > 0) {
            ...
            $.getScript("/Scripts/common/Attachment.js", function () {
                ...
                panelbody.append('<img class="left" id="empPhoto" style="cursor:pointer;margin-left:20px;width:150px;height:150px;" moduleName="员工管理" onclick="UploadFile(this)" num="1" src="' + img + '" onerror="javascript:this.src=\'/Images/Emp/male.jpg\'" alt="添加员工照片" title="添加员工照片">');
            });
            if (page == 'view') {
...
//字段值改变事件
function OnFieldValueChanged(moduleInfo, fieldName, vf, oldValue) {
    if (fieldName == 'OrgM_DeptId') {
        $('#OrgM_DutyId').combobox('reload', '/OrgM/GetDeptDutys.html?deptId=' + vf);
    }
}
